...
Under the **variant** section of your `workspace.json` file choose which components you want to have the custom environment set. You can find the id of the extension in the `dev.aspect.ts` file.

```json
{
  "teambit.workspace/variants": {
    "{ui/**}, {pages/**}": {
      "ashwanth1109.looper/envs/dev": {}
    }
  }
}
```
...
Bit uses ESLint to lint your components. You can add your own rules in the `dev.main.runtime` file.
...
Bit uses Prettier to format your components. You can add your own rules in the `dev.main.runtime` file.
...
Override the default dependencies in the `dev.main.runtime` file to include react types of a different version for example.
...
Extend the `dev.preview.runtime` file when you want to add your own customizations only for previewing in the Bit UI.
